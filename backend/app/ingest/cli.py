...
def ingest_corpus(corpus: Corpus, ctx: RefContext):
    """Returns (chunks, images_matched, images_unmatched_filenames)."""
    chunks = []
    images_matched = 0
    images_unmatched = []
    ...
    for ir in corpus.root.iter("image-reference"):
        chunk, matched_filename = parse_figure_file(ir, corpus)
        chunks.append(chunk)
        if matched_filename:
            images_matched += 1
        else:
    ...
    return chunks, images_matched, images_unmatched


def run(volume_two_source: Path, housing_source: Path, out_dir: Path) -> IngestReport:
    vol2 = load_corpus(
    ...
    for corpus in corpora:
        ...
        chunks, matched, unmatched = ingest_corpus(corpus, ctx)
...
def main(argv=None) -> int:
    ...
    report = run(volume_two_source, housing_source, args.out_dir)
    print_report(report)
